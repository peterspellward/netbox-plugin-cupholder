"""
Utility functions for testing NetBox Cup Holder Plugin.
...
def create_test_user(username: str | None = None, permissions: list[str | None] = None,
    ...
    Example:
        ...
        ...     permissions=['netbox_cup_holder_plugin.view_cupholder']
        ... )
    """
    if username is None:
...
def create_rack(name: str | None = None, site=None):
    """
    Create a minimal Rack for testing.

    Args:
        name: Rack name (defaults to random string)
        site: Site instance (creates one if not provided)
    ...
    if site is None:
        site = Site.objects.create(name=f'Site {get_random_string(8)}', slug=get_random_string(8).lower())

    if name is None:
        name = f'Rack {get_random_string(8)}'

    return Rack.objects.create(name=name, site=site)


def create_cupholder_type(
    name: str | None = None,
    size: str = 'm',
    ...
    """
    Create a CupholderType for testing.

    Args:
        name: Type name (defaults to random string)
        size: Size choice value
        material: Material description
        description: Type description

    Returns:
        CupholderType instance
    """
    from ..choices import CupholderSizeChoices
    from ..models import CupholderType

    if name is None:
        name = f'Type {get_random_string(8)}'

    if material is None:
        material = 'Stainless Steel'

    return CupholderType.objects.create(
        name=name,
        size=size or CupholderSizeChoices.SIZE_M,
        material=material,
        description=description,
    )


def create_cupholder(
    name: str | None = None,
    rack=None,
    cupholder_type=None,
    ...
    """
    Create a Cupholder for testing.

    Args:
        name: Cupholder name (defaults to random string)
        rack: Rack instance (creates one if not provided)
        cupholder_type: CupholderType instance (creates one if not provided)
        mount_face: Mount face choice value
    ...
    if cupholder_type is None:
        cupholder_type = create_cupholder_type()

    if name is None:
        name = f'Cupholder {get_random_string(8)}'

    return Cupholder.objects.create(
        name=name,
        rack=rack,
        cupholder_type=cupholder_type,
        mount_face=mount_face or CupholderMountFaceChoices.FACE_FRONT,
    )
...
def get_deletable_objects(instances):
    ...
    Example:
        >>> objs = Cupholder.objects.filter(name__startswith='test')
        >>> deletions = get_deletable_objects(objs)
        >>> # deletions = {'Cupholder': 3, 'RelatedModel': 5}
    """
...
def assert_object_changes(test_case, instance, action: str,
    ...
    Example:
        ...
        >>> obj = Cupholder.objects.create(name='Test')
